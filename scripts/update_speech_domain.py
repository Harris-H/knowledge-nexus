...
speech_kn_names = [
    ...
    "CTC (连接时序分类)",
    "声码器 (Vocoder)",
    "神经音频编解码 (Neural Audio Codec)",
    "语音大模型 (Speech Foundation Model)",
...
for name in speech_kn_names:
    cur.execute(
        "UPDATE knowledge_nodes SET domain = 'speech_ai' WHERE name = ?", (name,)
    )
...
for key in speech_paper_keys:
    row = cur.execute(
        "SELECT id, fields_of_study FROM papers WHERE key_contributions = ?", (key,)
    ).fetchone()
    if row:
        new_fos = "speech_ai," + (row[1] or "")
        cur.execute(
            "UPDATE papers SET fields_of_study = ? WHERE id = ?", (new_fos, row[0])
        )
        updated += 1
