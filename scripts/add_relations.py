...
NEW_RELATIONS = [
    ...
    (SRGAN, GAN, "BUILDS_ON", "SRGAN 将 GAN 的对抗训练框架应用于图像超分辨率任务"),
    (
        STABLE_DIFF,
        GAN,
        "RELATED_TO",
    ...
    (GAN_SURVEY, SRGAN, "RELATED_TO", "综述覆盖了 SRGAN 等条件 GAN 在图像恢复中的应用"),
    (
        GAN_SURVEY,
        STABLE_DIFF,
        "RELATED_TO",
    ...
    (NAT, VIT, "IMPROVES", "NAT 用邻域注意力替代全局注意力，降低 ViT 的计算复杂度"),
    (NAT, SWIN, "RELATED_TO", "都使用局部注意力机制，NAT 用邻域窗口，Swin 用移位窗口"),
    # === 注意力机制演化 ===
    (VIT, SENET, "BUILDS_ON", "ViT 的自注意力机制是 SE-Net 通道注意力思想的自然延伸"),
    (NAT, SENET, "BUILDS_ON", "邻域注意力继承了 SE-Net 局部特征增强的思路"),
    # === CIFAR 基准数据集 ===
    (GAN, CIFAR, "RELATED_TO", "GAN 论文使用 CIFAR-10 评估生成图像质量"),
    (
        VIT,
        CIFAR,
        "RELATED_TO",
    ...
    (SENET, CIFAR, "RELATED_TO", "SE-Net 在 CIFAR 上验证了通道注意力的效果"),
    (
        CONVNEXT,
        CIFAR,
        "RELATED_TO",
    ...
    # === 深度学习框架 & 工具 ===
    (VIT, PYTORCH, "RELATED_TO", "ViT 的主要开源实现基于 PyTorch (timm 库)"),
    (GAN, PYTORCH, "RELATED_TO", "GAN 及其变体广泛使用 PyTorch 实现和训练"),
    (
        NUMPY,
        TIDYVERSE,
        "ANALOGOUS_TO",
    ...
    # === 深度学习综述覆盖 ===
    (DL_SURVEY, VIT, "RELATED_TO", "综述覆盖了 Vision Transformer 等前沿视觉架构"),
    (DL_SURVEY, GAN, "RELATED_TO", "综述覆盖了 GAN 等生成模型的原理和发展"),
    (DL_SURVEY, MOCO, "RELATED_TO", "综述覆盖了自监督对比学习等无监督学习范式"),
    # === 自监督学习 & 表示学习 ===
    (
        MOCO,
        VIT,
    ...
    (DATA_AUG, GAN, "RELATED_TO", "GAN 生成的合成数据是重要的数据增强策略之一"),
    # === 跨模态/跨领域类比 ===
    (
        SENTENCE_BERT,
        VIT,
