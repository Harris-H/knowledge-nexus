...
对每条发现的关联，提供：
- source_name: 源知识节点/论文名称（必须是上面列表中存在的）
- target_name: 目标知识节点/论文名称（必须是上面列表中存在的）
...
def _node_to_text(name: str, node_type: str, domain: str,
                  summary: str, description: str) -> str:
    """将知识节点格式化为文本"""
    return f"[{node_type}|{domain}] {name}: {summary or description[:100]}"
...
async def discover_relations(db: AsyncSession, limit: int = 10) -> dict:
    ...
    for kn in kn_nodes:
        line = _node_to_text(kn.name, kn.node_type, kn.domain,
                             kn.summary or "", kn.description or "")
    ...
    for p in papers:
        ...
        line = f"[paper|computer_science] {name}: {p.summary or (p.abstract or '')[:100]}"
        nodes_text_lines.append(line)
    ...
    existing_relations_text = "\n".join(existing_text_lines[:30])  # 限制长度
    ...
    result = await chat_completion_json([
        {"role": "system", "content": "你是跨领域知识关联发现专家。请严格按照JSON格式返回结果。"},
    ...
    for item in items[:limit]:
        ...
        discoveries.append({
            ...
            "source_type": name_to_type.get(src_name, "knowledge_node"),
            "target_id": tgt_id,
            "target_name": tgt_name,
            "target_type": name_to_type.get(tgt_name, "knowledge_node"),
...
async def derive_knowledge(db: AsyncSession, node_ids: list[str]) -> dict:
    ...
    result = await chat_completion_json([
        {"role": "system", "content": "你是跨领域知识推导专家。请严格按照JSON格式返回深度分析结果。"},
        {"role": "user", "content": prompt},
    ], temperature=0.7, max_tokens=2000)
...
async def analyze_pair(db: AsyncSession, node_a_id: str, node_b_id: str) -> dict:
    ...
    result = await chat_completion_json([
        {"role": "system", "content": "你是跨领域知识关联分析师。请严格按照JSON格式返回分析结果。"},
        {"role": "user", "content": prompt},
...
async def save_discovered_relations(
    ...
    await db.flush()
    logger.info(f"💾 保存 {saved} 条 AI 发现的关联 (status={'confirmed' if auto_confirm else 'pending'})")
    return saved
